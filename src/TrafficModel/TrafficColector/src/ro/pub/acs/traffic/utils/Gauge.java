...
import ro.pub.acs.traffic.collector.R;
...
public final class Gauge extends View {
	...
	private static final int centerDegree = 100; // the one in the top center (12 o'clock)
	private static final int halfDegree = 50;
	...
	protected void onDraw(Canvas canvas) {
		...
		//bt = new BatteryTracer(this.getContext());
		//bt.start();
	...
	private float getRelativeTemperaturePosition() {
		if (handPosition < halfDegree) {
			return - (halfDegree - handPosition) / (float) (halfDegree - minDegrees);
		} else {
			return (handPosition - halfDegree) / (float) (maxDegrees - halfDegree);
		}
	}
