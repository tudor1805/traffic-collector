...
public class Utils {
	...
	public static double distance_haversine(double lat_a, double lon_a,
		...
		double a = Math.sin(deg2rad(alpha)) * Math.sin(deg2rad(alpha))
				+ Math.cos(deg2rad(lat_a)) * Math.cos(deg2rad(lat_b))
	...
	public static void sendPost(TrafficCollectorMain activity, String id,
			String surl, int type, String value) {
		try {
			Toast toast = null;
			...
			if (type == 0)
				dataOut.writeBytes("anti=10101&stop=" + value + "&id_user="
						+ id);
			else if (type == 1)
			...
			if (type == 0) {
				if (value.contains("1"))
					toast = Toast.makeText(activity,
							"Users cannot see your position anymore!",
							Toast.LENGTH_LONG);
				else if (value.contains("0"))
					toast = Toast.makeText(activity,
							"Users cann see your position again!",
							Toast.LENGTH_LONG);
				toast.show();
			} else if (type == 1) {
	...
	public static void sendPost(Activity activity, String id, String val1,
			String val2, String val3, String surl) {
		try {
			URL url = new URL(surl);
			HttpURLConnection connection = (HttpURLConnection) url
					.openConnection();
			...
			DataOutputStream dataOut = new DataOutputStream(
					connection.getOutputStream());
			dataOut.writeBytes("anti=10101&id_user=" + URLEncoder.encode(id)
					+ "&val1=" + URLEncoder.encode(val1) + "&val2="
					+ URLEncoder.encode(val2) + "&val3="
					+ URLEncoder.encode(val3));
			dataOut.flush();
	...
	public static void showDialog(TrafficCollectorMain activity,
			String errorMessage) {
		Toast toast = null;
		toast = Toast.makeText(activity, errorMessage, Toast.LENGTH_LONG);
		toast.show();
	}
