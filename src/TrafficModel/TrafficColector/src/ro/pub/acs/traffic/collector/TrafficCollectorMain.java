...
public class TrafficCollectorMain extends Activity {
	...
	private static final String stopShareURL = "http://"
			+ AppSettings.serverHost + "/traffic/stop.php";
	private static final String regenerateURL = "http://"
			+ AppSettings.serverHost + "/traffic/remove.php";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		share.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				i.putExtra(
						Intent.EXTRA_TEXT,
						"You can access my position: http://"
								+ AppSettings.serverHost
								+ "/traffic/view.php?id_user="
								+ (traffic.getId()));
				try {
		...
		regenerate.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				toast = Toast.makeText(thisActivity,
						"User id has been regenerated", Toast.LENGTH_SHORT);
				toast.show();
			}
		});

		wv = (WebView) findViewById(R.id.where);
		String content = "To view statistics follow:<br/> <a href=\"http://"
				+ AppSettings.serverHost + "/traffic\">http://"
				+ AppSettings.serverHost + "/traffic</a>";
		String html = "<html>"
				+ "<head></head>"
			...
			if (values.size() != 0) {
				...
				try {
					...
					while (!reply.equals("ACK")) {
						...
						String lname = db_user.isEmpty() ? "0" : db_user.get(
								"name").equals("") ? "0" : CryptTool
								.bytes2String(Base64.decode(
										db_user.get("name"), Base64.NO_WRAP));
						String lusername = db_user.isEmpty() ? "0" : db_user
								.get("username").equals("") ? "0" : db_user
								.get("username");
						String lpassword = db_user.isEmpty() ? "0" : db_user
								.get("password").equals("") ? "0" : ct
								.encrypt(db_user.get("password"));

						out.println("#s#" + lname + "#"
								+ (facebook.equals("") ? "0" : facebook) + "#"
								+ (twitter.equals("") ? "0" : twitter) + "#"
								+ traffic.getId() + "#" + lusername + "#"
								+ lpassword + "#"
					...
					for (int i = 0; i < values.size(); i++) {
						...
						out.println(value.get(1) + " " + value.get(2) + " "
				...
				/*
				 * String toSend = elements.toString(); try { URL url = new
				 * URL("http://" + AppSettings.serverHost +
				 * "/MACollector/collector_1.4.php"); HttpURLConnection
				 ...
				 * DataOutputStream(connection.getOutputStream()); Calendar cal
				 * = Calendar.getInstance(); SimpleDateFormat sdf = new
				 * SimpleDateFormat(DATE_FORMAT_NOW); String filename =
				 * "journey" + URLEncoder.encode(sdf.format(cal.getTime()),
				 * "UTF-8") + ".txt"; String sentName = URLEncoder.encode(name,
				 * "UTF-8"); String sentFacebook = URLEncoder.encode(facebook,
				 * "UTF-8"); String sentTwitter = URLEncoder.encode(twitter,
				 * "UTF-8"); dataOut.writeBytes("name=" + sentName +
				 * "&facebook=" + sentFacebook + "&twitter=" + sentTwitter +
				 * "&filename=" + filename + "&elements=" +
				 * URLEncoder.encode(toSend, "UTF-8")); dataOut.flush();
				 * 
				 * BufferedReader dataIn = new BufferedReader(new
				 * InputStreamReader(connection.getInputStream())); String line
				 * = dataIn.readLine(); if(line == null) { error = true;
				 * toastHandler.post(toastRunnableError); db.close(); } else
				 * if(!line.equals("200") ) { error = true;
				 * toastHandler.post(toastRunnableError); db.close(); }
				 * dataOut.close(); dataIn.close(); } catch (Exception e) {
				 * error = true; toastHandler.post(toastRunnableError);
				 * db.close(); e.printStackTrace(); } if(!error) {
				 * toastHandler.post(toastRunnableFinish); removeData(db);
				 * db.close(); }
				 */
			} else {
